...
def brute_force_matcher(img1, img2):
  ...
  image_with_matches = cv2.drawMatches(img1, kp1, img2, kp2, matches[:100], None, flags=cv2.DrawMatchesFlags_NOT_DRAW_SINGLE_POINTS)
  plt.imshow(image_with_matches)
  plt.savefig(matched_dir + '/' + L[k])
  #plt.show()
  return kp1_pix_coords, kp2_pix_coords #returns keypoints coords in xy [pix] for each image -call recursively
...
def brute_force_matcher_sift(img1, img2): #probable bug in distance method!
  ...
  image_with_matches = cv2.drawMatchesKnn(img1, kp1, img2, kp2, good, None, flags=cv2.DrawMatchesFlags_NOT_DRAW_SINGLE_POINTS)
  plt.imshow(image_with_matches)
  plt.savefig(matched_dir + '/' + L[k])
  plt.show()
  return kp1_pix_coords, kp2_pix_coords #returns keypoints coords in xy [pix] for each image -call recursively
...
def get_kp_coords(K, kp_pix_coords):
  ...
  return K_inv, kp_coords #returns camera inv and keypoints coords in xy [m] for each image -call recursively
...
def get_keypoint_depth(kp_pix_coords, img):
  ...
  return kp_depths #returns keypoint's depth z vector in [m] for each image -call recursively
...
def compute_pose_ransac(P_fixed, P_moving, min_inliers=50):
  ...
  for ransac_iter in range(0, 100): #do 100 iters
    inlier_indices = np.random.choice(P_fixed.shape[0], 6, replace=False)
    fixed_inliers = P_fixed[inlier_indices, :]
    #print(' fixed inliers  ', fixed_inliers)
    moving_inliers = P_moving[inlier_indices, :]
    #print('moving inliers = ', moving_inliers)
    X_guess, _ = do_icp(fixed_inliers, moving_inliers)
    total_error = 0.0
    inliers = inlier_indices.tolist()
    #for p_f, p_m in zip(P_fixed, P_moving):
    for i in range(P_fixed.shape[0]):
      p_f = P_fixed[i,:]
      p_m = P_moving[i, :]
      error = np.linalg.norm(- p_f + X_guess[0:3,0:3].T @ (p_m - X_guess[0:3,3][np.newaxis].T))
    ...
    if len(inliers) >= min_inliers*P_fixed.shape[0]: 
      if total_error < best_error:
        best_error = total_error
        #best_X = X_guess
        best_X, chi_lst = do_icp(P_fixed[inliers,:], P_moving[inliers,:])
        #plt.plot(chi_lst)
        #plt.show()
...
def error_and_jacobian(X, P_fixed, P_moving): #X=X_initial
  J = np.zeros((3, 6))
  J[0:3, 0:3] = np.eye(3)
  J[0:3, 3:6] = -skew(P_moving) #-skew(P_moving)
  J = -(X[0:3,0:3]).T @ J 
  #error = P_fixed - X @ P_moving
  #print('p moving = ', P_moving)
  #print('X = ',  X[0:3,3][np.newaxis].T)
  error = - P_fixed + X[0:3,0:3].T @ (P_moving - X[0:3,3][np.newaxis].T)
  return error, J
...
if __name__ == "__main__":
  ...
  for k, _ in enumerate(L[:-1]):
    ...
    P_fixed = get_point_in_camera_frame(kp1_coords, kp1_depths) #P1_vector = P_fixed
    P_moving = get_point_in_camera_frame(kp2_coords, kp2_depths) #P2_vector = P_moving
    ...
    best_X, total_error = compute_pose_ransac(P_fixed, P_moving, min_inliers=.5)
    X_vector.append(best_X)
    print('best_X = ', best_X)
